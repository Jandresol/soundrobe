...
create index if not exists product_search_cache_provider_query_idx
  on public.product_search_cache (provider, query);

create index if not exists product_search_cache_expires_at_idx
  on public.product_search_cache (expires_at);
